...
int acc_calibration_value = -236;//9706;                            //Enter the accelerometer calibration value

//Various settings
float pid_p_gain = 30; //Tu = 0.8s                                       //Gain setting for the P-controller (15)
float pid_i_gain = 0.5;//45;//1.5;                                      //Gain setting for the I-controller (1.5)
float pid_d_gain = .1;// 3;                                       //Gain setting for the D-controller (30)
...
int angleIncrementGain = 4;

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Setup basic functions
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void setup() {
  Serial.begin(38400);                                                       //Start the serial port at 9600 kbps
...
void loop() {
  ...
  angle_acc = atan((float)accelerometer_data_raw / 8192.0) * 57.296;       //Calculate the current angle according to the accelerometer
  ...
  angle_gyro += gyro_roll_data_raw * 0.0000611;                             //Calculate the traveled during this loop angle and add this to the angle_gyro variable
  ...
  if (pid_i_mem > 400)pid_i_mem = pid_i_gain * pid_error_temp;                                      //Limit the I-controller to the maximum controller output
  else if (pid_i_mem < -400)pid_i_mem = pid_i_gain * pid_error_temp;
  ...
  if (angle_gyro > 15 || angle_gyro < -15 || start == 0 ) {  //If the robot tips over or the start variable is zero or the battery is empty
  ...
  if (received_byte & B00000100) {                                          //If the third bit of the receive byte is set change the left and right variable to turn the robot to the right
    if (pid_setpoint > -2.5)pid_setpoint -= 0.05*angleIncrementGain;                           //Slowly change the setpoint angle so the robot starts leaning forewards
    if (pid_output > max_target_speed * -1)pid_setpoint -= 0.005*angleIncrementGain;           //Slowly change the setpoint angle so the robot starts leaning forewards
  }
  if (received_byte & B00001000) {                                          //If the forth bit of the receive byte is set change the left and right variable to turn the robot to the right
    if (pid_setpoint < 2.5)pid_setpoint += 0.05*angleIncrementGain;                            //Slowly change the setpoint angle so the robot starts leaning backwards
    if (pid_output < max_target_speed)pid_setpoint += 0.005*angleIncrementGain;                //Slowly change the setpoint angle so the robot starts leaning backwards
  }

  if (!(received_byte & B00001100)) {                                       //Slowly reduce the setpoint to zero if no foreward or backward command is given
    if (pid_setpoint > 0.5)pid_setpoint -= 0.05*angleIncrementGain;                            //If the PID setpoint is larger then 0.5 reduce the setpoint with 0.05 every loop
    else if (pid_setpoint < -0.5)pid_setpoint += 0.005*angleIncrementGain;                      //If the PID setpoint is smaller then -0.5 increase the setpoint with 0.05 every loop
...
ISR(TIMER2_COMPA_vect) {
  ...
  if (throttle_counter_left_motor > throttle_left_motor_memory) {           //If the number of loops is larger then the throttle_left_motor_memory variable
    ...
    if (throttle_left_motor_memory < 0) {                                   //If the throttle_left_motor_memory is negative
      PORTD |= 0b00100000;                                                  //Set output 5 high for a forward direction of the stepper motor
      throttle_left_motor_memory *= -1;                                     //Invert the throttle_left_motor_memory variable
    }
    else PORTD &= 0b11011111;                                             //Set output 5 low to reverse the direction of the stepper controller
  }
  ...
  if (throttle_counter_right_motor > throttle_right_motor_memory) {         //If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if (throttle_right_motor_memory < 0) {                                  //If the throttle_right_motor_memory is negative
      PORTD &= 0b10111111;                                               //Set output 6 high for a forward direction of the stepper motor
      throttle_right_motor_memory *= -1;                                    //Invert the throttle_right_motor_memory variable
    }
    else PORTD |= 0b01000000;                                                  //Set output 6 low to reverse the direction of the stepper controller
  }
  else if (throttle_counter_right_motor == 1)PORTD |= 0b00001000;           //Set output 3 high to create a pulse for the stepper controller
  else if (throttle_counter_right_motor == 2)PORTD &= 0b11110111;           //Set output 3 low because the pulse only has to last for 20us
}
